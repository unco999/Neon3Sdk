...
old = '''    /// Approximate scroll clamping without font access (viewport in "rows"
...
new = '''    /// Keeps scroll offsets non-negative after zoom / wheel events. Precise
...
assert old in d, 'clamp def not found'
d = d.replace(old, new)
open(p, 'w', newline='\n').write(d)
print('clamp fixed')
